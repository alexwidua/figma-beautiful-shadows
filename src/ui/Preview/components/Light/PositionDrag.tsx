...
import { stepped, deriveXYFromAngle } from "../../../../utils/math";
...
const PositionDrag = ({ children, style, ...rest }: { style?: any; children: any }) => {
  /**
   * 💾 Store
   */
  const { previewBounds, azimuth, position, distance, size, positionPointerDown, setLight } = useStore((state) => ({
  ...
  const drag: any = useDrag(
    ({ down: positionPointerDown, shiftKey: shiftKeyDown, offset: [ox, oy] }) => {
      const alignX = previewBounds.width / 2 - size / 2;
      const alignY = previewBounds.height / 2 - size / 2;
      const snapToGrid = {
        x: stepped(ox, GRID_SIZE) - GRID_SIZE + alignGridToCenter(previewBounds.width, GRID_SIZE),
        y: stepped(oy, GRID_SIZE) - GRID_SIZE + alignGridToCenter(previewBounds.height, GRID_SIZE),
      };
      const value = shiftKeyDown ? snapToGrid : { x: ox, y: oy };
      const { position, alignment } = align(value.x, value.y, alignX, alignY, LIGHT_SNAP_TO_AXIS_TRESHOLD);
      const data: Pick<Light, "x" | "y" | "alignment" | "positionPointerDown" | "shiftKeyDown"> = {
        ...position,
        alignment,
        positionPointerDown,
        shiftKeyDown,
  ...
  /**
   * 👂 Listen for azimuth and position changes and update position.
   */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setLight({ x: adjustedX, y: adjustedY });
  }, [distance]);
  ...
  useEffect(() => {
    ...
    if (outOfBoundsX || outOfBoundsY) {
      const x = outOfBoundsX ? previewBounds.width - LIGHT_SIZE : position.x;
      const y = outOfBoundsY ? previewBounds.height - LIGHT_SIZE : position.y;
