...
export function getCastedShadows({
  ...
  const shadows: DropShadowEffect[] | InnerShadowEffect[] = Array.from({ length: numShadows }, (_, i) => {
    ...
    const x = round(Math.cos(azimuthRad) * (scale * elevation * easeOffset));
    const y = round(Math.sin(azimuthRad) * (scale * elevation * easeOffset));

    // radius/ shadow blur
    const easeRadius = easeQuadOut(normalize(i, 0, numShadows));
    const radius = round(SHADOW_BASE_BLUR * increaseRadiusWithDistance * easeRadius * (elevation * 2));
  ...
  // due to the way the easing function works, we'll always end up with an almost transparent 0px 0px 0px 0px shadow. we'll just filter that one out...
  let _shadows: any[] = shadows;
  let filtered = _shadows.filter((shadow) => !(shadow.offset.x === 0 && shadow.offset.y === 0 && shadow.radius === 0));
  return filtered as DropShadowEffect[] | InnerShadowEffect[];
}
